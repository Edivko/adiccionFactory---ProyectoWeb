...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: ../comprador/calificar-vendedor.php');
    exit;
}
...
try {
    ...
    mysqli_stmt_bind_result($stmt, $idVendedor);
...
try {
    $stmtEx = mysqli_prepare($conexion,
        'SELECT id_calificacion_vendedor FROM CalificacionVendedor WHERE id_cita = ? LIMIT 1'
    );
...
try {
    $stmtIns = mysqli_prepare($conexion, '
        INSERT INTO CalificacionVendedor
            (id_comprador, id_vendedor, id_cita, puntuacion, comentario)
        VALUES (?, ?, ?, ?, ?)
    ');
    ...
    $_SESSION['mensaje_exito'] = 'Calificación del vendedor enviada correctamente. ¡Gracias por tu reseña!';
    header('Location: ../comprador/calificar-vendedor.php');
    exit;
